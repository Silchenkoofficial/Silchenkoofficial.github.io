...
class JSTikTok {
  ...
  force_download(url, title, format) {
    ...
    xhr.onload = function (e) {
      ...
      blob = new Blob(url, { type: "video/mp4" });
  ...
  get = async () => {
    ...
    var resp = await fetch(tiktokUrl)
    ...
    if (resp.includes(patterns[0])) {
      ...
      resp = await fetch(this.bypassCorsHeaders + this.urlencode(tiktokUrl))
        .then((response) => response.text())
        .then((data) => {
          return data;
        });
    ...
    var json = JSON.parse(
      resp.split(patterns[4])[1].split(patterns[3])[0].split(patterns[2])[1]
    );
    ...
    var author = {
      ...
      followers:
        json.props.pageProps.itemInfo.itemStruct.authorStats.followerCount,
      followings:
        json.props.pageProps.itemInfo.itemStruct.authorStats.followingCount,
  ...
  download_video = async () => {
    ...
    this.force_download(
      this.bypassCorsHeaders +
        this.urlencode(this.datas.video.download_url) +
        "&d=1",
      this.datas.video.id,
      "mp4"
    );
